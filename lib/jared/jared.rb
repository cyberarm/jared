...
class Jared < Thor
    ...
    def server
      if File.exists?("#{Dir.home}/.jared") && File.directory?("#{Dir.home}/.jared")
        unless File.exists?("#{Dir.home}/.jared/server.pid")
          File.open("#{Dir.home}/.jared/server.pid", 'w') do |f|
            f.write("#{Time.now}\n0.0.7a4")
          end
        else
        ...
        log.info "Created '.jared' directory in #{Etc.getlogin}'s home folder."
        self.server
      end
      require 'drb'
      require_relative 'drb/drb_server.rb'
      Action::Drb.start_server
    end
